...
# This dataset contains hourly pollution readings (PM2.5 concentration) from five cities in China: Beijing, Guangzhou, Shenyang, Chengdu, and Shanghai, over the course of several years.  In Shanghai, for instance, there are >20k readings, accompanied by several features
...
# From previous run, for comparability, set to None to re-tune
lambda_list = {
  (0, 1): 40.0,
  (0, 2): 40.0,
  (0, 3): 40.0,
...
for CITY in cities:
    ...
    for test_season in seasons:
        ...
        for k, est in estimators.items():
            if 'tune_lambda' in est.keys() and est['tune_lambda']:
                if lambda_list is not None:
                    best_lambda = {
                        'pred__lamb': lambda_list[(CITY, test_season)]
                    }
        ...
        ar_types = ['AR', 'xAR', 'OLS']
        tar_types = ['TAR', 'xTAR', 'OLS']

        rmse_df = pd.concat(rmse_dfs, axis=0)

all_res_df = pd.concat(all_res_dfs, axis=0)
all_rmse_df = pd.concat(all_rmse_dfs, axis=0)
all_res_df.to_csv(f"results/all_res_test_anchor_prox_{anchor}_lamb_fixed.csv")
all_rmse_df.to_csv(f"results/all_rmse_test_anchor_prox_{anchor}_lamb_fixed.csv")
